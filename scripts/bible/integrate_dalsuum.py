#!/usr/bin/env python3
"""
Integrate  Zolai-English-Myanmar Dictionary
Reads data/online/-zolai-dictionary/words.json (7,861 headwords)
Cross-checks against existing dict_zo_en_clean.jsonl (93K entries)
Outputs: data/dictionary/processed/dict__merged.jsonl
"""
...
INPUT_FILE = DATA_DIR / "online" / "-zolai-dictionary" / "words.json"
EXISTING_DICT = DATA_DIR / "dictionary" / "processed" / "dict_zo_en_clean.jsonl"
OUTPUT_FILE = DATA_DIR / "dictionary" / "processed" / "dict__merged.jsonl"
...
# Synset ID → POS mapping (from  data)
SYNSET_POS = {
...
def main():
    print(f"[integrate_] Loading  dictionary from: {INPUT_FILE}")
    ...
    print(f"[integrate_] Loading existing dictionary: {EXISTING_DICT}")
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as out:
        for sense in senses:
            ...
            entry = {
                ...
                "myanmar_word": "",  #  doesn't separate Myanmar
                "pos": pos,
                "source": "",
                "sense": sense_text,
                "example": example,
