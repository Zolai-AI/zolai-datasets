...
cmd_integrate_() {
  banner
  echo -e "${G}Integrating  dictionary (7,861 words)...${NC}"
  $PYTHON "$SCRIPT_DIR/integrate_.py" "$@"
  echo ""
  read -p "Press Enter to return to menu..."
}
...
cmd_extract_corpus_vocab() {
  banner
  echo -e "${G}Extracting vocabulary from  corpus (208MB)...${NC}"
...
cmd_extract_myanmar() {
  echo -e "${C}═══ Extract Myanmar from  ═══${NC}"
  echo ""
  echo -e "  Extracting ZO→MY pairs from  dictionary..."
  cd "$WORKSPACE"
  PYTHONPATH="$WORKSPACE/zolai-core" $PYTHON zolai-datasets/scripts/my/extract_myanmar.py 2>&1 | tail -5
  echo ""
  echo -e "  ${G}Done!${NC} Output: data/processed/my/dict_zo_my_.jsonl"
  echo ""
  read -p "Press Enter to return to menu..."
}
...
cmd_search_myanmar_dict() {
  ...
  if [ ! -f "$dict_file" ] && [ ! -f "$en_my_file" ]; then
    ...
    echo -e "  Run M1 first to extract from ."
    read -p "Press Enter to return to menu..."
    return
  fi
...
while true; do
  ...
  echo -e "  ${G}Q${NC}) 🔗 Integrate  dictionary"
  ...
  echo -e "  ${G}M1${NC}) 📖 Extract Myanmar from  dict"
