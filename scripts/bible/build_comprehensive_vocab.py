...
Merges: vocab_by_frequency + vocab_comprehensive + corpus online +  + dict_zo_en
...
#  dictionary
_DICT = DATA_DIR / "dictionary" / "processed" / "dict__merged.jsonl"
...
def load__vocab(filepath: Path) -> dict:
    """Load  entries as {word: translation}."""
    vocab = {}
    if not filepath.exists():
        return vocab
    with open(filepath, "r", encoding="utf-8") as f:
...
def main():
    ...
    for word, freq in corpus_freqs.items():
        if word in master:
            ...
            master[word]["sources"].add("_corpus")
        else:
            master[word] = {
                "word": word,
                "translation": "",
    ...
    print("\n  Loading  dictionary...")
    _vocab = load__vocab(_DICT)
    print(f"     words: {len(_vocab):,}")
    new_from_ = 0
    for word, trans in _vocab.items():
        if word in master:
            master[word]["sources"].add("")
            ...
            new_from_ += 1
    print(f"    Added {new_from_:,} new words from ")
