...
Ingests ALL raw and processed data into the zolai.db database:
  1. Dictionary enrichment (verified, corrections, , bible, zomidaily)
...
SOURCE_FILES: dict[str, dict[str, Any]] = {
    ...
    "dict__merged": {
        "path": DICT_DIR / "dict__merged.jsonl",
        "description": " trilingual dictionary (7.8K)",
    },
    "dict_bible_combined": {
...
class DataIngester:
    ...
    def ingest_dictionary_enrichment(self) -> None:
        ...
        # 1c.  merged → fill missing myanmar + add new entries
        print("\n  [1c] dict__merged.jsonl → dictionary.myanmar + new entries")
         = load_jsonl(SOURCE_FILES["dict__merged"]["path"])
        if :
            _map: dict[str, dict] = {}
            for rec in :
            ...
            self.stats["dict_"] = {"myanmar_filled": my_count}
        ...
        self._record_provenance("dict__merged",
                                SOURCE_FILES["dict__merged"], len())
