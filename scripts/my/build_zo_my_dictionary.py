...
Reads the extracted  ZO→MY data and joins with the canonical
...
# Input paths
_MY_PATH = (
    WORKSPACE / "data" / "processed" / "my" / "dict_zo_my_.jsonl"
)
ZO_EN_MASTER_PATH = (
...
def load_zo_my() -> dict[str, str]:
    """Load  ZO→MY, dedup by headword (take first sense)."""
    headwords: dict[str, str] = {}
    if not _MY_PATH.exists():
        print(f"ERROR: {_MY_PATH} not found", file=sys.stderr)
        sys.exit(1)
