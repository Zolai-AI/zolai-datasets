"""Extract Myanmar (Burmese) data from  Zolai Dictionary.

Reads data/online/-zolai-dictionary/words.json and extracts
senses containing Myanmar Unicode script (\u1000-\u109F).

Output:
    data/processed/my/dict_zo_my_.jsonl  — ZO→MY entries
...
_PATH = (
    WORKSPACE / "data" / "online" / "-zolai-dictionary" / "words.json"
)
OUTPUT_DIR = WORKSPACE / "data" / "processed" / "my"
OUTPUT_JSONL = OUTPUT_DIR / "dict_zo_my_.jsonl"
OUTPUT_REPORT = OUTPUT_DIR / "myanmar_source_report.json"

# Synset mapping from  words.json
SYNSET_MAP = {
...
def extract_myanmar_senses(
    ...
    """Extract senses containing Myanmar Unicode from the  dictionary."""
    results: list[dict] = []
...
def main() -> int:
    ...
    if not _PATH.exists():
        print(f"ERROR: Source file not found: {_PATH}", file=sys.stderr)
        return 1
