...
Fills missing Myanmar translations for vocabulary entries.
...
async def get_missing_entries(limit: int = 500) -> List[Dict]:
    """Get vocabulary entries missing Myanmar translations."""
    ...
    cursor = conn.execute("""
        ...
        AND headword IS NOT NULL AND headword != ''
...
async def translate_with_ensemble(client: ZolaiGeminiOpenAIClient, word: str, english: str) -> Dict:
    """Translate using ensemble voting."""
    prompt = f"""Translate to Myanmar (Burmese): {word} = {english}. Return ONLY the Myanmar word."""
    ...
    for model in ENSEMBLE_MODELS:
        try:
            ...
            print(f"  Warning: {model} failed for {word}: {e}")
...
async def process_batch(limit: int = 500):
    ...
    for i, entry in enumerate(entries):
        ...
        # Resume check
        c.execute("SELECT myanmar FROM vocabulary WHERE headword = ?", (word,))
        row = c.fetchone()
        if row and row[0]:
            continue

        print(f"\n[{i+1}/{total}] {word} ({english})")

        result = await translate_with_ensemble(client, word, english)
        ...
        if result["translation"]:
            ...
            c.execute("""
                ...
                WHERE headword = ? AND (myanmar IS NULL OR myanmar = '')
            """, (result["translation"], word))

            if c.rowcount > 0:
                translated += 1
                log_audit(
                    conn, "vocabulary", word, "myanmar",
                    old_myanmar or "", result["translation"],
                    f"batch_fix_vocabulary_myanmar: conf={result['confidence']:.2f}"
                )
...
async def main():
    import argparse
    parser = argparse.ArgumentParser(description="Batch Myanmar translation for vocabulary table")
